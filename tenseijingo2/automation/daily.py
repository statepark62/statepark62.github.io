    ...
    if sample_mode:
        ...
        try:
            ...
            if not naver.env_ready():
                ...
                page_url = f"https://statepark62.github.io/tenseijingo/{today.isoformat()}.html"
                subject = f"[天声人語 학습] {today.month}월 {today.day}일 — {data['topic_ko']}"
                content = (
                    f"{data['topic_ja']}\n{data['topic_ko']}\n\n"
                    f"{data['one_line_ko']}\n\n"
                    f"오늘의 학습 페이지 (요약·단어·문형·해설):\n{page_url}\n\n"
                    f"전체 목록: https://statepark62.github.io/tenseijingo/"
                )
