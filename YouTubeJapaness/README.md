...
`index.html` 상단의 "새 영상에서 표현 추출하기" 패널에서, GitHub Actions에
직접 들어가지 않고도 유튜브 URL을 넣어 실행할 수 있습니다.

**GitHub 토큰 준비 (최초 1회)**
1. [GitHub 토큰 발급 페이지](https://github.com/settings/tokens?type=beta)에서
   "Generate new token"을 누릅니다.
2. Repository access를 "Only select repositories"로 하고
   `statepark62.github.io` 저장소만 선택합니다.
3. Permissions에서 **Actions: Read and write**만 켭니다 (다른 권한은 필요
   없습니다).
4. 만료 기간을 적당히(예: 90일) 설정하고 발급받습니다.
5. 발급된 토큰(`github_pat_...`)을 복사해둡니다.

**사용법**: 패널을 열고 URL, (채널이면) 최대 영상 수, 토큰을 넣고
"지금 추출 실행"을 누르면 됩니다. 토큰은 **이 브라우저 탭에만** 잠깐
저장되고(세션 종료 시 삭제), 저장소나 서버 어디에도 기록되지 않습니다.
같은 탭에서는 다시 안 넣어도 되지만, 탭을 닫으면 다시 입력해야 합니다.

**주의**: 이 토큰은 저장소에 대한 쓰기 권한을 가진 민감한 정보입니다.
공용 컴퓨터에서는 사용을 피하시고, 만료 기간이 지나면 다시 발급받아
주세요.
